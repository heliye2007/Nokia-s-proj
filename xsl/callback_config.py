...
def send_variable(k, v):
    ...
    tn.sock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
    tn.read_until('login:')
    tn.write(username + '\r\n')
...
def install_by_pip(pack_name, aggressive=True):
    d = dict(os.environ)
    d['https_proxy'] = 'https://10.144.1.10:8080'
    p = subprocess.Popen(['C:/Python27/Scripts/pip.exe', 'uninstall', pack_name], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    print p.communicate('y\r\n')[0]
    time.sleep(5)

    cmd_list = ['C:/Python27/Scripts/pip.exe', 'install', pack_name]
    if aggressive:
        cmd_list.insert(2, '--pre')
    p1 = subprocess.Popen(cmd_list, close_fds=True, env=d)
    p1.wait()


def config_bts():
    map(lambda x: install_by_pip(x), ['pycrypto', 'robotframework','robotframework-ride'])
